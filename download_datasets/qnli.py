...
with open(train_path, 'r', encoding='utf-8') as f:
    for line_id, line in enumerate(f):
        if line_id == 0:
            continue
        index, question, sentence, label = line.rstrip().split('\t')
        train_text_a.append(question)
        train_text_b.append(sentence)
        if label == 'entailment':
            label = 1
        else:
            label = 0
...
with open(dev_path, 'r', encoding='utf-8') as f:
    for line_id, line in enumerate(f):
        if line_id == 0:
            continue
        index, question, sentence, label = line.rstrip().split('\t')
        valid_text_a.append(question)
        valid_text_b.append(sentence)
        if label == 'entailment':
            label = 1
        else:
            label = 0
